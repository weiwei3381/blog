...
1. **/model**: 模型(model)类, 存放实体类文件
   ...
   2. `Zone.js`: 区域类, 内部有个`_gridPool`属性, 用来放置每个网格, 每个网格有自己的默认属性(网格的颜色, 对角坐标, 网格初始参数), 也可以传入用户参数, 绘制时使用
...
2. **/storage**: 存储类
   ...
   2. `environment.js`: 环境类, 存放所有实例化后的区域/目标/威胁.
3. **/shape**: 存放常用形状
   1. `base.js`: 形状基类
   2. 其他形状类, 主要有矩形\三角形\多边形\五角星\椭圆形
4. **/handler**: 控制器(controller)类, 存放各种无人机相互之间, 和环境之间交互的文件
   1. `strategy.js`: 设置无人机的各类策略, 传入环境参数, 判断无人机下一个要去的地点
   2. `interaction.js`: 交互类, 威胁和其他无人机对当前无人机的影响, 增加力到`_forceList`中去, 更新网格/威胁/目标的各类情况
...
8. **/event**: 事件相关目录
   ...
   2. `Dispatcher`: 事件分发类
9. /: 根目录
10. `simulation.js`: 仿真类, 主类, 提供 Simulation 类, 并实例化 simulation.
