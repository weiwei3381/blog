...
每台无人机所处的地理位置不同，所感知的环境也可能不同。因此，每个无人机的ω也不同。而且，每一个无人机在同一时刻只能执行一个任务，因此
...
因此，该问题可以被定义为一个LP问题，每个UAV都要求解自己的LP。优化问题改写为：
...
式中，VTj=目标Tj值，wr=搜索任务对攻击目标任务的权重，St=（到达目标时间Tj)/（总飞行时间）。然而，第i架无人机以一定的概率知道目标j的值。假设概率分布是线性的，如图1所示。设pr(dj)定义目标j在距离d处具有r值的概率。这里，r={0,0.5,1}其中，当r=1时，目标不受攻击，当r=0.5时，目标被部分摧毁，当r=0时，目标被完全摧毁。因此，Cij是概率p(dj)={pr(dj)}的随机变量。

![wJdZIf.png](https://s1.ax1x.com/2020/09/10/wJdZIf.png)

投机/BDA：由于对目标的投机是在每一个时间步进行的，并且反映在目标的价值上，所以我们没有为投机任务附加单独的利益。

每个无人机还估计其相邻的无人机（如第$k$个无人机）将从其可以执行的不同任务中获得的好处如下：

搜索任务：搜索任务与上述定义相似，因此所有无人机的搜索值相同。

攻击目标j：如果目标j在kth无人机的传感器半径内，则：

![wgmh1.png](https://s1.ax1x.com/2020/09/10/wGmmH1.png)

如果目标j不在第k架无人机的传感器范围内，则Ckj=0。在这里，我们假设所有的无人机具有相同的传感器范围，因此第i架无人机可以估计第j个目标是否在第k架无人机的传感器范围内。

攻击虚拟目标：虚拟目标的概念用于估计第i架无人机传感器范围以外的环境（见图2)。

![wJd8Zq.png](https://s1.ax1x.com/2020/09/10/wJd8Zq.png)

第i架无人机无法看到第j架无人机可以看到的阴影区域。根据阴影区域内目标的数量，jth无人机的行为将有所不同。为了估计可能存在的目标数量，我们假设目标是均匀分布的。我们考虑了所有这些目标的综合效应，我们假设这些目标位于一个点p,与点(a,b)等距。这种组合目标被称为kth无人机的虚拟目标。kth无人机攻击这个虚拟目标的好处是：

![wGml9O.png](https://s1.ax1x.com/2020/09/10/wGml9O.png)

其中，nk是阴影区域中可以出现的目标数量。因此，nk=ni（阴影区面积）/π(sr)2,Clˆk=0,∀l=1,...，ni l=k,sr是传感器范围。也就是说，对于任何其他无人机，攻击第k架无人机的虚拟目标ˆk的好处是零。

由于CIJ是随机变量，第i架无人机将使预期收益最大化。期望值是根据状态的联合概率Pi(s)计算的。在这里，我们假设目标值是独立事件，因此

![wGmYDA.png](https://s1.ax1x.com/2020/09/10/wGmYDA.png)

目标是在第二节定义的约束条件下使预期收益E（ω）最大化，因此每个无人机解决了以下线性规划问题：
